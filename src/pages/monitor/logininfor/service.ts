...
export async function getLogininforList(params?: LogininforListParams) {
  ...
  return request(`${SYSTEM_PATH}/logininfor/list?${queryString}`, {
    data: params,
    method: 'GET',
    headers: {
...
export function getLogininfor(infoId: number) {
  return request(`${SYSTEM_PATH}/logininfor/${infoId}`, {
    method: 'GET',
  });
}
...
export async function addLogininfor(params: LogininforType) {
  return request(`${SYSTEM_PATH}/logininfor`, {
    method: 'POST',
    data: params,
  });
}
...
export async function removeLogininfor(ids: string) {
  return request(`${SYSTEM_PATH}/logininfor/${ids}`, {
    method: 'DELETE',
    headers: {
      'Content-Type': 'application/json;charset=UTF-8',
    },
...
export function exportLogininfor(params?: LogininforListParams) {
  return downLoadXlsx(`${SYSTEM_PATH}/logininfor/export`, {params}, `login_infor_${new Date().getTime()}.xlsx`);
}

// 清空登录日志
export async function cleanLogininfor() {
  return request(`${SYSTEM_PATH}/logininfor/clean`, {
    method: 'DELETE',
  });
}
